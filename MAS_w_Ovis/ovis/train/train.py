...
def load_model(model_args: ModelArguments, training_args: TrainingArguments):
    # Determine torch_dtype based on training args
    torch_dtype = None
    ...
    if training_args.freeze_vision_encoder:
        for param in model.visual_tokenizer.vit.parameters():
            param.requires_grad = False
        rankN_print("Vision encoder frozen")
    
    if training_args.freeze_text_encoder:
        # Freeze text embedding layer
        for param in model.llm.get_input_embeddings().parameters():
            param.requires_grad = False
        rankN_print("Text encoder (embedding layer) frozen")
...
def train(model_args: ModelArguments, training_args: TrainingArguments):
    ...
    # Debug: Check which modules have trainable parameters
    llm_trainable = sum(p.numel() for p in model.llm.parameters() if p.requires_grad)
    vit_trainable = sum(p.numel() for p in model.visual_tokenizer.vit.parameters() if p.requires_grad)
    vte_trainable = sum(p.numel() for p in model.vte.parameters() if p.requires_grad)
    total_trainable = sum(p.numel() for p in model.parameters() if p.requires_grad)
    
    rank0_print(f'Trainable parameters:')
    rank0_print(f'  LLM: {llm_trainable:,} parameters')
    rank0_print(f'  ViT: {vit_trainable:,} parameters')  
    rank0_print(f'  VTE: {vte_trainable:,} parameters')
    rank0_print(f'  Total: {total_trainable:,} parameters')
