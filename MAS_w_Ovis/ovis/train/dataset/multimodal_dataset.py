...
class DataCollatorForMultimodalDataset:
    def __init__(self, text_tokenizer: PreTrainedTokenizer, training_args=None):
    ...
    def __call__(self, instances: Sequence[Dict]) -> Dict[str, torch.Tensor]:
        ...
        # Add MAS loss arguments if available
        if self.training_args is not None:
            if hasattr(self.training_args, 'use_mas_loss'):
                batch_dict['use_mas_loss'] = self.training_args.use_mas_loss
            if hasattr(self.training_args, 'mas_loss_weight'):
                batch_dict['mas_loss_weight'] = self.training_args.mas_loss_weight
            if hasattr(self.training_args, 'mas_tau'):
                batch_dict['mas_tau'] = self.training_args.mas_tau
            if hasattr(self.training_args, 'mas_only'):
                batch_dict['mas_only'] = self.training_args.mas_only
